...
func (permDB *PermissionDB) From(perm model.Permission) {
	...
	permDB.Roles = steams.Map(steams.FromSlice(perm.Roles), func(role model.Role) RoleDB {
		var roleDB RoleDB
		roleDB.From(role)
		return roleDB
	}).Collect()
...
func (permDB PermissionDB) Into() model.Permission {
	return model.Permission{
		ID:   permDB.ID,
		Name: permDB.Name,
		Roles: steams.Map(steams.FromSlice(permDB.Roles), func(roleDB RoleDB) model.Role {
			return roleDB.Into()
		}).Collect(),
	}
}
